...
export const generateTasksForCollector = async (collector, todayInfo, transaction = null) => {
  ...
  // Use a separate transaction per demand so one validation/DB error does not abort the whole batch
  for (const demand of eligibleDemands) {
    const ownerId = demand.property?.ownerId ?? null;
    if (ownerId == null) {
      skipped.push({ demandId: demand.id, reason: 'Property has no owner' });
      continue;
    }

    const demandTxn = await sequelize.transaction();
    try {
      // Get or create follow-up
      let followUp = await FollowUp.findOne({
        where: { demandId: demand.id },
        transaction: demandTxn
      });
      ...
      const sequence = String(existingTasksCount + tasksGenerated + 1).padStart(3, '0');
