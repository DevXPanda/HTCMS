...
import { Bath, Download, RefreshCw, Search, FileDown } from 'lucide-react';
...
const SBMToilet = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await api.get('/toilet/facilities', { params });
      const facilities = res.data?.data?.facilities ?? [];
      setList(Array.isArray(facilities) ? facilities : []);
    } catch (err) {
      toast.error(err.response?.data?.message || 'Failed to load toilet facilities');
      setList([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = () => {
    const rows = list.map((f) => ({
      ...
      type: f.type,
      status: f.status,
      ward: f.ward?.wardName ?? f.wardId
    }));
    exportToCSV(rows, `sbm_toilet_facilities_${new Date().toISOString().slice(0, 10)}`);
    toast.success('CSV downloaded');
  };

  return (
    <div className="space-y-4">
      <h1 className="print-only text-xl font-bold text-gray-900 mb-2">Toilet Management</h1>
      <div className="no-print">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <Bath className="w-7 h-7 text-teal-600" />
          Toilet Management (Read-only)
        </h1>
        <p className="text-gray-600 text-sm">View toilet facilities across ULBs. Filter by ULB, search, export.</p>
      </div>
      ...
      <div className="no-print flex flex-wrap gap-3 items-end">
        ...
        <form onSubmit={handleSearch} className="flex gap-2 flex-1 min-w-0 max-w-md">
          <div className="relative flex-1 min-w-0">
            ...
            <input type="text" placeholder="Search by name, location..." value={search} onChange={(e) => setSearch(e.target.value)} className="input pl-9 w-full" />
          </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
        {loading && !list.length ? (
          <div className="p-8 flex justify-center"><RefreshCw className="w-8 h-8 animate-spin text-teal-500" /></div>
        ) : (
              ...
              <tbody className="divide-y divide-gray-200">
                {list.map((f) => (
                  <tr key={f.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">{f.ward?.wardName ?? f.wardId ?? '—'}</td>
