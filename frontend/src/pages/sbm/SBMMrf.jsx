...
import { Recycle, Download, RefreshCw, Search, FileDown } from 'lucide-react';
...
const SBMMrf = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await api.get('/mrf/facilities', { params });
      const facilities = res.data?.data?.facilities ?? res.data?.facilities ?? [];
      setList(Array.isArray(facilities) ? facilities : []);
    } catch (err) {
      toast.error(err.response?.data?.message || 'Failed to load MRF facilities');
      setList([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = () => {
    ...
    exportToCSV(rows, `sbm_mrf_facilities_${new Date().toISOString().slice(0, 10)}`);
    toast.success('CSV downloaded');
  };

  return (
    <div className="space-y-4">
      <h1 className="print-only text-xl font-bold text-gray-900 mb-2">MRF Management</h1>
      <div className="no-print">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <Recycle className="w-7 h-7 text-emerald-600" />
          MRF Management (Read-only)
        </h1>
        <p className="text-gray-600 text-sm">View MRF facilities across ULBs. Filter by ULB, search, export.</p>
      </div>
      ...
      <div className="no-print flex flex-wrap gap-3 items-end">
        ...
        <form onSubmit={handleSearch} className="flex gap-2 flex-1 min-w-0 max-w-md">
          <div className="relative flex-1 min-w-0">
            ...
            <input type="text" placeholder="Search..." value={search} onChange={(e) => setSearch(e.target.value)} className="input pl-9 w-full" />
          </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
        {loading && !list.length ? (
          <div className="p-8 flex justify-center"><RefreshCw className="w-8 h-8 animate-spin text-emerald-500" /></div>
        ) : (
