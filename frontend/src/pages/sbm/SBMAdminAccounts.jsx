...
import { Shield, Download, RefreshCw, Search, FileDown, Eye } from 'lucide-react';
import toast from 'react-hot-toast';

const SBM_ULB_STORAGE_KEY = 'htcms_sbm_selected_ulb_id';

const SBMAdminAccounts = () => {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const params = { role: 'admin', limit: 10000 };
  ...
  const handleExport = () => {
    ...
    exportToCSV(rows, `sbm_admin_accounts_${new Date().toISOString().slice(0, 10)}`);
  ...
  const ulbName = (id) => ulbs.find((u) => u.id === id)?.name || id || '—';

  return (
    <div className="space-y-4">
      <h1 className="print-only text-xl font-bold text-gray-900 mb-2">Admin Management</h1>
      <div className="no-print">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <Shield className="w-7 h-7 text-violet-600" />
          Admin Management (Read-only)
        </h1>
        <p className="text-gray-600 text-sm">View admin accounts — same as Super Admin. SBM view-only.</p>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
            ...
            <tbody className="divide-y divide-gray-200">
              {filtered.map((u) => (
                <tr key={u.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600">{u.ulb_id ? ulbName(u.ulb_id) : '—'}</td>
                  ...
                  <td className="px-4 py-3 print-hide-col">
                    <Link to={`/sbm/admin-accounts/${u.id}`} className="text-violet-600 hover:text-violet-800 flex items-center gap-1 text-sm">
        ...
        {!loading && filtered.length === 0 && (
          <div className="p-8 text-center text-gray-500">No admin accounts found.</div>
        )}
      </div>
    </div>
  );
...
export default SBMAdminAccounts;
