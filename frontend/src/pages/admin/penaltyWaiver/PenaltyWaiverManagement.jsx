...
const amountClass = {
  ...
  waived: 'text-amber-600 font-semibold',
  ...
  summary: 'text-amber-700 font-bold'
};
...
const resetFormBelow = (setters) => {
  ...
  setters.setWaiverValue('');
  setters.setReason('');
  setters.setDocumentUrl('');
};

const PenaltyWaiverManagement = () => {
  ...
  useEffect(() => {
    penaltyWaiverAPI.getSummary()
      .then((res) => res.data?.success && res.data?.data && setSummary(res.data.data))
      .catch(() => setSummary(null))
      .finally(() => setLoadingSummary(false));
  }, []);

  useEffect(() => {
    penaltyWaiverAPI.getHistory({ limit: 50 })
  ...
  const handleDownloadPdf = async (id) => {
    setPdfLoadingId(id);
    try {
      const res = await penaltyWaiverAPI.getPdf(id);
      const url = URL.createObjectURL(res.data);
      const a = document.createElement('a');
      a.href = url;
      a.download = `penalty-waiver-${id}.pdf`;
      a.click();
  ...
  const handleViewModalDownload = async () => {
    ...
    try {
      const res = await penaltyWaiverAPI.getPdf(viewModalRow.id);
      const url = URL.createObjectURL(res.data);
      const a = document.createElement('a');
      a.href = url;
      a.download = `PenaltyWaiver_Receipt_${viewModalRow.demandNumber || viewModalRow.id}.pdf`;
      a.click();
      URL.revokeObjectURL(url);
  ...
  useEffect(() => {
    ...
    demandAPI
      .getByModuleEntity(module, entityId, { forPenaltyWaiver: '1' })
      .then((res) => setDemands(res.data.data?.demands || []))
      .catch(() => {
  ...
  const demandsWithPenalty = demands;

  const originalAmount = selectedDemand ? getDemandOriginalAmount(selectedDemand) : 0;
  const penaltyAmountTotal = selectedDemand ? getDemandPenaltyAmount(selectedDemand) : 0;
  ...
  const waiverValNum = parseFloat(waiverValue) || 0;
  const { waiverAmount: computedWaiver, remainingPenalty, error: waiverError } = calculatePenaltyWaiver(penaltyAmountTotal, waiverType, waiverValNum);
  const previewFinal = selectedDemand
    ? calculateFinalAmount(selectedDemand, { discountAmount: 0, waiverAmount: computedWaiver })
    : null;
  const finalPayable = previewFinal ? previewFinal.finalAmount : 0;
  const existingDiscount = selectedDemand && selectedDemand.taxDiscounts?.length
    ? Number((selectedDemand.taxDiscounts.reduce((s, d) => s + parseFloat(d.discountAmount || 0), 0)).toFixed(2))
    : 0;
  const isWaiverFormDisabled = selectedDemand && penaltyAmountTotal <= 0;
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      const res = await penaltyWaiverAPI.uploadDocument(formData);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!documentUrl) {
      toast.error('Please upload the approval/application PDF or image');
      return;
    }
    if (penaltyAmountTotal <= 0) {
      toast.error('Demand has no penalty or interest to waive');
      return;
    }
  ...
  const handleDetailDownloadPdf = async () => {
    ...
    try {
      const res = await penaltyWaiverAPI.getPdf(createdAdjustment.id);
      const url = URL.createObjectURL(res.data);
      const a = document.createElement('a');
      a.href = url;
      a.download = `PenaltyWaiver_Receipt_${createdAdjustment.demandNumber || createdAdjustment.id}.pdf`;
      a.click();
      URL.revokeObjectURL(url);
      toast.success('Receipt downloaded');
    } catch (err) {
    ...
    <div className="space-y-8 max-w-7xl mx-auto px-2">
      ...
      <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        {loadingSummary ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-white rounded-xl border border-amber-100 shadow-sm p-5 hover:shadow-md transition-all">
                <div className="p-2.5 rounded-lg bg-amber-600 text-white w-fit mb-2">
                  <Shield className="h-5 w-5" />
                </div>
                <p className="text-sm font-medium text-gray-500">Total Waivers Applied</p>
                <p className="text-xl font-bold text-gray-900">{summary?.totalWaivers ?? 0}</p>
              </div>
              <div className="bg-white rounded-xl border border-emerald-100 shadow-sm p-5 hover:shadow-md transition-all">
                <div className="p-2.5 rounded-lg bg-emerald-600 text-white w-fit mb-2">
                  <IndianRupee className="h-5 w-5" />
                </div>
                <p className="text-sm font-medium text-gray-500">Total Waiver Amount (Current FY)</p>
                <p className={`text-xl ${amountClass.summary}`}>{formatCurrency(summary?.totalWaiverAmountFY)}</p>
              </div>
              <div className="bg-white rounded-xl border border-blue-100 shadow-sm p-5 hover:shadow-md transition-all">
                <div className="p-2.5 rounded-lg bg-blue-600 text-white w-fit mb-2">
                  <FileText className="h-5 w-5" />
                </div>
                <p className="text-sm font-medium text-gray-500">Active Waived Demands</p>
                <p className="text-xl font-bold text-gray-900">{summary?.activeWaivedDemands ?? 0}</p>
              </div>
              <div className="bg-white rounded-xl border border-violet-100 shadow-sm p-5 hover:shadow-md transition-all">
                <div className="p-2.5 rounded-lg bg-violet-600 text-white w-fit mb-2">
                  <Calendar className="h-5 w-5" />
                </div>
                <p className="text-sm font-medium text-gray-500">This Month Waivers</p>
                <p className="text-xl font-bold text-gray-900">{summary?.monthlyWaivers ?? 0}</p>
              </div>
            </div>
          </>
        )}
      ...
      {/* Penalty Waiver History - below summary */}
      <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="text-right py-3 px-4 font-semibold text-gray-700">Waived</th>
              ...
              <tbody>
                {history.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="py-8 text-center text-gray-500">No penalty waiver history yet.</td>
                  </tr>
                ) : (
                  history.map((row) => (
                    <tr key={row.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                      ...
                      <td className="py-3 px-4 text-gray-600">{row.waiverType === 'PERCENTAGE' ? `${row.waiverValue}%` : 'Fixed'}</td>
                      <td className={`py-3 px-4 text-right font-semibold ${amountClass.waived}`}>{formatCurrency(row.waiverAmount)}</td>
      ...
      {viewModalRow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 transition-opacity duration-200" onClick={() => setViewModalRow(null)}>
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Penalty Waiver Details</h3>
            <dl className="space-y-2 text-sm">
              ...
              <div><dt className="text-gray-500">Type</dt><dd className="font-medium">{viewModalRow.waiverType === 'PERCENTAGE' ? `${viewModalRow.waiverValue}%` : 'Fixed'}</dd></div>
              <div><dt className="text-gray-500">Waived Amount</dt><dd className="font-medium">{formatCurrency(viewModalRow.waiverAmount)}</dd></div>
            ...
            <div className="mt-6 flex justify-end gap-2 flex-wrap">
              <button type="button" onClick={handleViewModalDownload} disabled={viewModalPdfLoading} className="inline-flex items-center gap-1.5 px-4 py-2 bg-amber-600 text-white rounded-lg font-medium hover:bg-amber-700 disabled:opacity-50">
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 transition-all duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="flex flex-col items-center text-center">
              ...
              <div className="flex gap-3 w-full justify-center">
                <button type="button" onClick={handleSuccessYes} className="px-5 py-2.5 bg-amber-600 text-white rounded-lg font-medium hover:bg-amber-700 transition-colors">
                  Yes, View / Download
                </button>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 transition-opacity duration-200" aria-modal="true">
          <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-hidden flex flex-col transition-all duration-200">
            <div className="p-6 flex items-start justify-between gap-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">Penalty Waiver Details</h3>
              <button type="button" onClick={handleDetailDownloadPdf} disabled={detailDownloading} className="inline-flex items-center gap-1.5 px-3 py-2 bg-amber-600 text-white rounded-lg text-sm font-medium hover:bg-amber-700 disabled:opacity-50">
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <dl className="space-y-3 text-sm">
                ...
                <div><dt className="text-gray-500">Original Amount (Penalty)</dt><dd className={amountClass.original}>{formatCurrency(createdAdjustment.originalAmount)}</dd></div>
                <div><dt className="text-gray-500">{createdAdjustment.adjustmentLabel || 'Waived Amount'}</dt><dd className={amountClass.waived}>{formatCurrency(createdAdjustment.adjustmentAmount)}</dd></div>
      ...
      {/* Apply New Penalty Waiver - Step-based workflow */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-6 flex items-center gap-2">
          <FileText className="w-5 h-5 text-amber-600" />
          Apply New Penalty Waiver
        </h2>

        {/* Step 1: Tax module */}
        <section className="mb-8">
          <p className="text-xs font-medium text-amber-600 uppercase tracking-wide mb-2">Step 1 of 4</p>
          <h3 className="text-base font-medium text-gray-800 mb-2">Select tax module</h3>
          <select
            value={module}
            onChange={(e) => handleModuleChange(e.target.value)}
            className="w-full max-w-xl border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
          >
        ...
        {module && (
          <section className="mb-8 pt-6 border-t border-gray-100">
            <p className="text-xs font-medium text-amber-600 uppercase tracking-wide mb-2">Step 2 of 4</p>
        ...
        {module && entityId && (
          <section className="mb-8 pt-6 border-t border-gray-100">
            <p className="text-xs font-medium text-amber-600 uppercase tracking-wide mb-2">Step 3 of 4</p>
            <h3 className="text-base font-medium text-gray-800 mb-3">Select a demand (with penalty)</h3>
            {loadingDemands ? (
              <Loading />
            ) : (
              <div className="overflow-x-auto rounded-lg border border-gray-200">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50">
                      ...
                      <th className="text-right py-3 px-4 font-medium text-gray-700">Original Tax</th>
                      <th className="text-right py-3 px-4 font-medium text-gray-700">Penalty</th>
                      <th className="text-right py-3 px-4 font-medium text-gray-700">Remaining</th>
                      <th className="w-20 py-3 px-4"></th>
                    </tr>
                  </thead>
                  <tbody>
                    {demandsWithPenalty.map((d) => (
                      <tr key={d.id} className="border-t border-gray-100 hover:bg-gray-50/50">
                        <td className="py-3 px-4 font-medium">{d.demandNumber}</td>
                        <td className={`py-3 px-4 text-right ${amountClass.original}`}>{formatCurrency(parseFloat(d.totalAmount || 0) - parseFloat(d.penaltyAmount || 0) - parseFloat(d.interestAmount || 0))}</td>
                        <td className={`py-3 px-4 text-right ${amountClass.waived}`}>{formatCurrency(parseFloat(d.penaltyAmount || 0) + parseFloat(d.interestAmount || 0))}</td>
                ...
                {demandsWithPenalty.length === 0 && <p className="py-6 text-center text-gray-500 text-sm">No demands with penalty for this entity.</p>}
              </div>
            )}
          </section>
        )}

        {/* Step 4: Waiver form + Document + Submit - only after demand selected */}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <div className="flex gap-6">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="waiverType" checked={waiverType === 'PERCENTAGE'} onChange={() => setWaiverType('PERCENTAGE')} />
                    Percentage
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="waiverType" checked={waiverType === 'FIXED'} onChange={() => setWaiverType('FIXED')} />
                    Fixed amount
                  </label>
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Waiver value</label>
                <input
                  type="number"
                  min="0"
                  step={waiverType === 'PERCENTAGE' ? '0.01' : '1'}
                  max={waiverType === 'PERCENTAGE' ? 100 : penaltyAmountTotal}
                  value={waiverValue}
                  onChange={(e) => setWaiverValue(e.target.value)}
                  disabled={isWaiverFormDisabled || isFullyPaid}
                  className="border border-gray-300 rounded-lg px-4 py-2.5 w-full max-w-xs min-w-[12rem] disabled:opacity-60"
                />
                {waiverValue && waiverError && <p className="text-sm text-red-600 mt-1">{waiverError}</p>}
              </div>
              <div className={`p-4 rounded-lg border text-sm ${waiverError ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-100'}`}>
                <div className="flex justify-between py-1"><span className="text-gray-600">Penalty</span><span className={amountClass.original}>{formatCurrency(penaltyAmountTotal)}</span></div>
                <div className="flex justify-between py-1"><span className="text-gray-600">Waived</span><span className={amountClass.waived}>{formatCurrency(computedWaiver)}</span></div>
              ...
              <div>
                ...
                <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3} className="border border-gray-300 rounded-lg px-4 py-2.5 w-full min-w-0" placeholder="Enter reason for penalty waiver" />
                {!reason.trim() && <p className="text-sm text-amber-600 mt-1">Reason is required.</p>}
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Upload approval/application PDF or image (mandatory)</label>
                <input type="file" accept=".pdf,application/pdf,image/jpeg,image/png,image/webp" onChange={handleFileChange} className="border border-gray-300 rounded-lg px-4 py-2.5 w-full max-w-xl text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-amber-50 file:text-amber-700 file:font-medium" />
...
export default PenaltyWaiverManagement;
