...
const CollectorDemandDetails = () => {
  ...
  useEffect(() => {
    demandAPI
      .getById(id)
      .then((res) => {
        if (res.data.success && res.data.data.demand) setDemand(res.data.data.demand);
        else setDemand(null);
      })
      .catch((err) => {
  ...
  const downloadDiscountPdf = async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `discount-approval-${demand.taxDiscounts[0].id}.pdf`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (err) {
      ...
      {discount && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <dl className="divide-y divide-gray-100">
            <div className="px-5 py-3 flex justify-between"><dt className="text-sm text-gray-500">Original Amount</dt><dd className="font-semibold">{formatAmt(demand.totalAmount)}</dd></div>
            <div className="px-5 py-3 flex justify-between"><dt className="text-sm text-gray-500">Discount Applied</dt><dd className="font-semibold">{formatAmt(discount.discountAmount)}</dd></div>
            <div className="px-5 py-3 flex justify-between"><dt className="text-sm text-gray-500">Final Payable</dt><dd className="font-semibold">{formatAmt(demand.finalAmount ?? calculateFinalAmount(demand, { discountAmount: parseFloat(discount.discountAmount || 0), waiverAmount: parseFloat(demand.penaltyWaived || 0) }).finalAmount)}</dd></div>
          </dl>
          {showDiscountDetails && (
