        ...
        <header className="no-print bg-white shadow-sm sticky top-0 z-10 w-full">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <StaffPortalHeaderRow
              searchRole="inspector"
              dashboardPath="/inspector/dashboard"
              userInitial={userData?.firstName?.charAt(0) || userData?.full_name?.charAt(0) || 'I'}
              userTitle={userData?.firstName || userData?.full_name || 'Inspector'}
              ...
              <div className="modal-body space-y-4">
                <div className="flex items-center space-x-4 mb-6">
                  <div className="h-16 w-16 bg-primary-100 rounded-full flex items-center justify-center text-primary-600 text-xl font-bold">
                    {userData?.firstName?.charAt(0) || userData?.full_name?.charAt(0) || 'I'}
                  </div>
                  <div>
                    <h4 className="text-lg font-medium text-gray-900">
                      {userData?.firstName || userData?.full_name || 'Inspector'}
                    </h4>
                    <p className="text-sm text-gray-500 capitalize">{userData?.role || 'inspector'}</p>
                  </div>
                </div>
